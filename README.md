...
`world.obj` 메시와 텍스처를 로드해 **텍스처가 매핑된 3D 지구**를 렌더링하고, 자유 시점 카메라로 둘러볼 수 있다.
...
> `1` / `2` / `3` 키로 각 축 회전 토글. 왼손 좌표계 기준 왼손 규칙으로 통일.

### 짐벌락 현상

![짐벌락](Docs/Earth_짐벌락%20현상.gif)

> 현재 오일러 각(ZYX 순서) 기반이므로 두 축을 동시에 회전하면 짐벌락이 발생한다. 쿼터니언 도입 예정.
...
> 데모 모델·텍스처는 `Game/Asset/`에 동봉되어 있고, 빌드 후 실행 파일 옆으로 자동 복사됩니다.
> 클론 → 빌드 → 실행하면 별도 설정 없이 텍스처가 입혀진 3D 지구가 렌더링됩니다.
...
- **오일러 각 → 쿼터니언 전환 예정** — 현재 ZYX 오일러 순서로 회전하며, 두 축 동시 회전 시 짐벌락 발생. 쿼터니언 도입으로 해결 예정.
