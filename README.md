...
DX11 기반 4x4 행렬. Row-major, 왼손 좌표계.

- `union` 방식 — `m00~m33` 이름 접근 + `elements[16]` 순회 접근 공유
- 기본 생성자 — Identity 행렬
- `Transpose()` — 멤버 + static(`std::swap` 방식)
- `Determinant()` — 여인수 전개, `det3x3` private 헬퍼
- `Inverse()` — 여인수 + Adjugate(수반행렬) + `invDet` 방식
- `operator*(Matrix4)` — 행렬 곱, `operator*(Vector4)` — 벡터 변환
- `TransformPosition(Vector3)` — w=1, 이동+회전+스케일 적용
- `TransformVector(Vector3)` — w=0, 회전+스케일만 적용
- static factory (구현 예정): `Translation`, `Scale`, `RotationX/Y/Z`, `Rotation`, `LookAt`, `PerspectiveFOV`
