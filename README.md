...
- ⚠️ **Stride 설계 고민 필요** — `Render()`와 `InitShaders()`가 별도 함수로 분리되어 있어 `Vertex` 구조체를 공유할 수 없음. 현재 `sizeof(float) * 7`로 하드코딩 중. `Vertex` 구조체를 별도 분리하거나 stride를 멤버로 관리하는 방식 검토 필요.
