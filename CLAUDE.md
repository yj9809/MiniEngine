...
- 대화가 길어져 컨텍스트 압축이 예상되는 시점에, 기능 단위 완료가 확인되면 먼저 wrap-up을 제안한다.
  > "컨텍스트가 길어졌는데, 여기까지 wrap-up하고 다음 세션에서 이어갈까요?"
- wrap-up 제안 기준: 하나의 기능/버그픽스가 빌드 + 동작 확인까지 완료된 시점
- 기능이 미완성이더라도 "진행 중 🔄" 상태로 기록하고 마무리할 수 있다
