...
namespace Engine
{
    ...
    void WireframePass::Execute(ID3D11DeviceContext* context, const std::vector<RenderCommand>& commands,
        const std::unordered_map<BufferHandle, ComPtr<ID3D11Buffer>>& bufferMap)
    {
        ...
        // 렌더링 후 래스터라이저를 기본 상태로 복구.
        context->RSSetState(nullptr);
    }

    bool WireframePass::CreateConstantBuffer(ID3D11Device* device, UINT byteWidth, ComPtr<ID3D11Buffer>& buffer) const
    {
